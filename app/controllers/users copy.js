...
fields={
   ...
   roleId : "60a8a1cf64118b095870f90b",
  };
// Bcrypt.hashSync("india@123"),
    new userModel(fields)
        .save(function(err) {
            if (err) {
              if (err.name === 'MongoError' && err.code === 11000) {
                // Duplicate username
                return res.status(422).send({ succes: false, message: 'User already exist!' , error: err});
              }
    ...
    getUser: async (req, res) => {
        try {
            ...
            qq+=  JSON.stringify({accountType: req.query.accountType});
